...
import os, os.path, sys, imp, datetime, time, logging, socket, json, tempfile, itertools
import requests
...
def restart_ams_if_necessary(ambari_cluster, service):
    ...
    if service.is_ams_collector_host(ams_collector_host, this_host):
        try:
            log.info("Restarting AMS to make new whitelist metrics effective")
            with rm.Environment():
                rm.Execute([service.get_ams_collector_exe(), '--config', service.get_ams_collector_config(), 'restart'], user='ams', logoutput=True)
        except rm.Fail as ex:
            log.warning('Failed to successfully restart AMS metrics collector. Details: %s', ex.message)

def wait_for_cluster_installation(ambari_cluster, service):
    ...
    timeout_time = start_time + datetime.timedelta(seconds=service.get_installation_timeout())
    while datetime.datetime.utcnow() < timeout_time:
        # Make this call mockable
        requests = list(service.get_cluster_requests(ambari_cluster, request_context='Auto Start Host Components', request_status='COMPLETED'))
        if len(requests) > 0:
            ...
            return True
        time.sleep(service.get_delay_base_unit())
    log.fatal('FATAL: Timed out waiting for Ambari cluster installation to complete')
    return False

def wait_for_edge_node_scripts(ambari_cluster, num_edge_nodes, service, edgenode_script_tag='edgenode-signature-tag'):
    ...
    # Wait around for 30 mins
    timeout_time = datetime.datetime.utcnow() + datetime.timedelta(seconds=service.get_installation_timeout())
    edge_node_hosts = set()
    while len(edge_node_hosts) < num_edge_nodes:
        edge_node_hosts = set()
        # Make this call mockable
        custom_action_requests = service.get_cluster_requests(ambari_cluster, request_context='run_customscriptaction', request_status='COMPLETED')
...
def wait_for_requests_to_complete(ambari_cluster, service):
    ...
    while datetime.datetime.utcnow() < timeout_time:
        # Make this call mockable
        requests = list(service.get_cluster_requests(ambari_cluster, request_status='IN_PROGRESS'))
        if len(requests) == 0:
...
def make_ambari_service_effective(service_name, service):
    ...
    time.sleep(service.get_delay_base_unit() * 2)
    ...
    time.sleep(service.get_delay_base_unit() * 3)
...
class MockableService(object):
    ...
    def get_stack_service_base_dir(self, stack_name, stack_version):
        if _platform == 'linux' or _platform == 'linux2':
            return '/var/lib/ambari-server/resources/stacks/{0}/{1}/services'.format(stack_name, stack_version)
        else:
            return os.path.join(tempfile.gettempdir(), 'services')
